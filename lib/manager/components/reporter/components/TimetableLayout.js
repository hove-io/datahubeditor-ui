    ...
    return (
      <div>
        <Row style={{ marginBottom: '20px' }}>
          <Col xs={12} md={6}>
            <label htmlFor='route_name'>Ligne:</label>
            <Select
              options={routes.data || []}
              labelKey={'route_name'}
              valueKey={'route_id'}
          ...
          {routeFilter &&
            <Col xs={12} md={6}>
              <label htmlFor='pattern'>Itinéraire:</label>
              <Select
        ...
        {!allFiltersSelected &&
          <Alert>
            Sélectionner une ligne et un itinéraire
          </Alert>
        }
        ...
        {allFiltersSelected && timetables.fetchStatus.fetched && timetableTableData.rows.length === 0 &&
          <Alert bsStyle='warning'>
            Pas de services trouvés pour la sélection de ligne, d'itinéraire, de date et de plage horaire.
          </Alert>
        }
      </div>
    )
