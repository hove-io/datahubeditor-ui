...
export default class EditShapePanel extends Component<Props> {
  ...
  async drawPatternFromStops (pattern: Pattern, stopsCoordinates: Array<LatLng>, followStreets: boolean): Promise<any> {
    ...
    if (patternSegments && patternSegments.length > 0) {
      ...
      setErrorMessage({message: 'Erreur lors de la création de l\'itinéraire depuis les arrêts. Certains arrêts doivent être inaccessible par les rues.'})
      return false
    }
  }
  ...
  _confirmCreateFromStops = () => {
    const title = 'Créer un tracé d\'itinéraire depuis les arrêts ?'
    const onConfirm = this._generateShapeFromStops
    const body = this._hasShapePoints()
      ? 'Êtes-vous sûr de vouloir remplacer le service précédent pour cet itinéraire ?'
      : 'Êtes-vous sûr de vouloir créer un tracé auto-généré pour cet itinéraire ?'
  ...
  _deleteShape = () => {
    ...
    showConfirmModal({
      title: `Supprimer le tracé pour cet itinéraire ?`,
      body: `Êtes-vous sûr de vouloir supprimer ce tracé d'itinéraire (shape_id: ${shapeId})?`,
      onConfirm: () => {
  ...
  render () {
    ...
    const fromStopsButton = <OverlayTrigger
      placement='bottom'
      overlay={
        <Tooltip id='from-stops'>Générer le tracé d'itinéraire depuis les arrêts</Tooltip>
      }>
      <Button
        ...
        <div style={{margin: '5px 0'}}>
          {!activePattern.shapeId
            ? <small className='text-warning'>
              ...
              Pas de tracé associé à cet itinéraire.
            </small>
            : <small>
        ...
        {patternStopsWithShapeIssues.length > 0
          ? <Alert bsStyle='warning' style={{fontSize: 'small'}}>
            ...
            <p>
              ...
              mètres) du tracé d'itinéraire.
            </p>
            <p>
              Ce problème peut être résolu en
              <ol>
                <li>
                  déplaçant l'arrêt en le mettant plus proche de la rue,
                </li>
                <li>
                  éditant le tracé, ou
                </li>
                <li>
                  regénérant le tracé (cliquer sur{' '}
