...
export default class VirtualizedEntitySelect extends Component<Props, State> {
  ...
  render () {
    ...
    let disabled = false
    let placeholder = `Sélectionner ${component}...`
    let options = []
    if (entities.length > 30000) {
      console.warn(`Liste trop longue (${entities.length} choix). Menu déroulant désactivé.`)
      disabled = true
      placeholder = 'Menu déroulant désactivé. Liste trop longue.'
    } else {
      options = entities
        .filter(e => {
          if (e.id < 0) {
